...
for line_index in range(0, number_on_first_line):
    ...
    space_station_position, time_limit = line[0], line[1]
    print("NOW AT LINE", line)
    movement_sequence = generate_movement_sequence(space_station_position)

    print("WE GOT", movement_sequence)
    print("TOTAL TIME:", calculate_time(movement_sequence))

    if line_index == len(lines_of_file) - 1:
        movement_sequences.append(" ".join([str(i) for i in movement_sequence]))
    else:
        movement_sequences.append(" ".join([str(i) for i in movement_sequence]) + "\n")
...
#
# f = open("level3_0_example.out", "w")
# f = open("level3_1_small.out", "w")
f = open("level3_2_large.out", "w")
